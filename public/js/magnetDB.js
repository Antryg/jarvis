        ...
        queue: function(self, item) {
            if (item.queue) {
                return <div className="switch tiny round" onClick={self.triggerQueue.bind(self, item)}>
                    <input id={"queue_" + item.id} checked type="checkbox" />
                    <label htmlFor={"queue_" + item.id}></label>
                </div>;
            } else {
                return <div className="switch tiny round" onClick={self.triggerQueue.bind(self, item)}>
                    <input disabled id={"queue_" + item.id} type="checkbox" />
                    <label htmlFor={"queue_" + item.id}></label>
                </div>;
            }
        },
        series: function(self, data) {
